...
hora の各パッケージが配る全ファイル — skill も agent も — は、エージェントが動く前に実行時に読まれます。**8 文かけて述べた規則は、毎回 8 文ぶんの費用がかかります。**
...
**書き写す前に、まず参照を明確にします。** 参照にはファイル、見出し、そこで読者が何を見つけるかを書きます。その参照でも読者が動けない場合に限って規則を書き写し、規則そのものは所有ファイルに残します。

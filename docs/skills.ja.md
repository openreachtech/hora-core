...
hora-skills-ort-* 「stub 実装は server/<area>/<audience>/stub/ に置く」
                       ...
                       │  パッケージが更新される。パスが変わる。
                       ▼
Hora Kit          「stub 実装は server/<area>/<audience>/stub/ に置く」
                       ↑
                  そのまま、自信満々のまま、間違いになる
```
...
| パッケージ | プレフィックス | 守備範囲 | カタログ |
|---|---|---|---|
| `hora-skills-ort-core` | `hoc-` | スタックを問わず、あらゆるプロジェクトに効く規約と手順 | [カタログ](https://github.com/openreachtech/hora-skills-ort-core/blob/main/docs/skills.ja.md) |
| `hora-skills-ort-renchan` | `hor-` | バックエンドの面 | [カタログ](https://github.com/openreachtech/hora-skills-ort-renchan/blob/main/docs/skills.ja.md) |
| `hora-skills-ort-furo` | `hof-` | フロントエンドの面 | [カタログ](https://github.com/openreachtech/hora-skills-ort-furo/blob/main/docs/skills.ja.md) |
| `hora-skills-ort-support` | `hos-` | コードの周りの仕事 — 結果を説明する、利用者が読むマニュアルを書く | [カタログ](https://github.com/openreachtech/hora-skills-ort-support/blob/main/docs/skills.ja.md) |

**各パッケージは自分のスキルを自分で文書化しており、このページはそれを再掲しません。** ここに一覧を置けば、それは全カタログの2つ目の写しになり、スキルが1つ増えた/改名された瞬間から食い違い始めます — このドキュメント全体が扱っているのは、まさにその失敗です。

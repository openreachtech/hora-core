...
| **API スキーマと操作** | audience ごとのスキーマ、サーバーエンジン、操作の resolver、入力バリデータ、共有の resolver コンテナ、**stub resolver** |
...
| **テストキャッシュ** | 入力が変わっていない実行の記録済み結果の再利用と、再利用した結果に何を添えて報告するか |
| **git** | コミット規約 |

...
2つの半分は、必要とする注意の量も違います。そして推奨される走らせ方は、その違いに従っています。 `/hora-spec` はステージごとに付き添う価値があります — 最初から最後まで対話であり、仕様書が「機能名の一覧」で終わらなくなる場所だからです。実装の側は走らせておけます — **答えが要るときは、決めずに止まる**ので。自動執行がこちらでは安全で、もう片方ではそうでない理由は、それです。[`hora-boilerplate` の `README.ja.md`](https://github.com/openreachtech/hora-boilerplate/blob/main/README.ja.md#推奨仕様は対話で実装は自動執行で) の「推奨：仕様は対話で、実装は自動執行で」を参照してください。
...
**3つめは、どちらのスケジューラも人なしで走らせます — `/hora-auto`。** 仕様書がステージ7を通ったあとに、何を重視するかを一度だけ宣言してもらい、経路が尋ねていたはずのことをすべて決めて記録しながら、`main` への draft の pull request まで進みます。マージは人に残ります。マージがリリースだからです。経路は [`auto.ja.md`](./auto.ja.md) にあります。
...
退行の網が累積であることが、上の表の3行目を成立させています。 [`/hora-accept`](../kit/skills/hora-accept/SKILL.md) はどの関所でもユニットスイートをリポジトリ全体に対して流します。したがって、ある機能が過去の機能を壊した場合、壊したその実行で落ちます。高コストな側 — 実ブラウザで画面を駆動するレビュー — は関所ではその機能自身に絞られ、全機能を端から端まで駆動するのは版全体のスイープ（または明示的に要求した実行）の仕事です。
...
並列タスクごとにブランチを与えれば解決します — ただし**1つの作業ディレクトリは同時に1本しかチェックアウトできず**、`/hora` は git worktree を使っていません。同じ制約は実行中にも再来します。依存が途中で見つかったとき、直列ならそのタスクだけ止めて導入し、rebase すれば済みますが、並列では開いている複数のブランチがそれぞれ rebase を要求し、それは編集中の何かの足元で作業ディレクトリごと切り替えることを意味します。

**worktree 無しでは、直列は用心深い既定値ではなく、正しくコミットできる唯一の順序です。**

**`/hora-fast` がその解決で、代価は別の場所で払います。** 機能ごとに専用の git worktree を持つので、機能ごとにブランチ、コミット、rebase を持てます。その代価 — 共有ファイルを先に建てること、機能ごとのデータベース、回帰の特定が関所ではなくマージの時点になること — は [`parallel.ja.md`](./parallel.ja.md) にあります。既定は `/hora` のままです。回帰をコミット1つ分の新しさで見つける順序だからです。

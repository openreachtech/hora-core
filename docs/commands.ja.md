...
**通常の利用では `/hora` しか打ちません。** どれを走らせるかは `/hora` が決めます。**`/hora` が起動しないものが2つあります — `/hora-hotfix` と `/hora-fast`** です。緊急かどうか、そして期日のために並列で建てるかどうかは、人が決めます。残りを記載しているのは、直接呼びたい場面があるためです — 検収をやり直したい、仕様変更後に計画だけ見たい、途中で終わったセットアップを直したい、など。
...
| **単独で叩く** | 常に。`/hora` はこれを起動しません — 期日のために並列で建てるかどうかは人が決めることです |

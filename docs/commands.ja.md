...
主要な6つのコマンドを、毎回同じ5項目で説明します：何をするか / 何を読むか / 何を書くか / いつ止まるか / 単独で叩きたいとき。このほかに直接叩けるものとして、`/hora-hotfix`（緊急経路。後述）、`/hora-fast`（並列経路。後述）、`/hora-auto`（自動経路。後述）と、`/hora-spec` が走らせる7つのステージ skill（`/hora-spec` の節に名前があります）があります。

**通常の利用では `/hora` しか打ちません。** どれを走らせるかは `/hora` が決めます。**`/hora` が起動しないものが3つあります — `/hora-hotfix`、`/hora-fast`、`/hora-auto`** です。緊急かどうか、並列で建てる価値があるかどうか、そして誰も答えないまま建ててよいかどうかは、人が決めます。残りを記載しているのは、直接呼びたい場面があるためです — 検収をやり直したい、仕様変更後に計画だけ見たい、途中で終わったセットアップを直したい、など。

**このうち2つは人が付いていてほしく、残りは走らせておけます。** `/hora-spec` は最初から最後まで対話で、`/hora-plan` は仕様が未決のまま残した所を尋ねます。`/hora-setup` / `/hora-build` / `/hora-accept` に見張りは要りません — **決めずに止まって尋ねる**ので、放っておいても安全です。推奨と、「自動執行」が意味すること・意味しないことは [`hora-boilerplate` の `README.ja.md`](https://github.com/openreachtech/hora-boilerplate/blob/main/README.ja.md#推奨仕様は対話で実装は自動執行で) の「推奨：仕様は対話で、実装は自動執行で」にあります。
...
6. 全機能が終わり、_sweep.md の最新ブロックが、どの release/<version> も
   今なお指す tips: の上での reach: full の合格でない
                                           → /hora-accept（版全体）
7. そうである                              → main へ merge
```
...
**3つのどれであっても、「あなたにとって何が変わるか」の言葉で出されます。** 「このカラムをあちらに紐づけます。いいですか」と訊かれても、カラムの話しか答えられません。「1つの見出しにぶら下がる項目を一括で書き込めるようになります」なら、同じ決定をあなたが判断できる場所で訊かれています。それを実現する設計は、そのあとに一節だけ添えられます。観測できる差が何も生じないもの — リポジトリ名やミドルウェアのバージョン — は、そう断ったうえで技術的な言葉のまま訊かれます。
...
| | 扱い |
|---|---|
| **未リリース** | 追加も変更も削除も受け付け、**版番号は変わりません。** 利用者がいないので、contract を変えても誰も壊れません。起きるのは手戻りであって、互換性の破壊ではありません。リリース直前の仕様変更は、まったく正常です |
...
hotfix を止める性質は6つ — 後方互換でないスキーマ変更、開いている `release/<version>` のマイグレーションと同じテーブルに触るスキーマ変更、contract の変更、依存の変更、新しい operation / 画面 / ジョブ、1本のブランチに収まらない作業。
...
**並列スケジューラ。** 機能が共有するファイルを先に一度だけ建て、そのあと複数の機能を同時に、それぞれ専用の git worktree で建てます。経路の全体は [`parallel.ja.md`](./parallel.ja.md) にあり、ここはその要約です。仕様書、計画、機能ファイル、検収は `/hora` と同じで、変わるのは順序だけです。

| | |
|---|---|
| **読む** | `/hora` が読むものすべて。加えて、宣言された全リポジトリの `git worktree list` |
| **書く** | `/hora` と `/hora-build` が書くものすべてを同じ形で。加えて `.hora/tasks/<version>/_fast.md` — 誰が選んだか、上限、土台 |
| **止まる条件** | `/hora` が止まる条件。2つめの作業コピーに必要なものの答えが stack handbook に無い。ゲート途中の機能を worktree へ移す手順が失敗した |
| **単独で叩く** | 常に。`/hora` はこれを起動しません — 並列で建てる価値があるかどうかは人が決めることです |

### 何をするか

```
0-4. /hora と同じ。そのうえで、main の作業コピーが feature/ ブランチにいるか？
                                   → その機能を worktree へ移す
5.   土台は建っているか？          いいえ → 建てる：2つの機能が両方書くファイルを、
                                            一度だけ release/<version> に
6.   まだ [ ] の機能があるか？     はい → 上限まで worktree を開き、/hora-build と
                                          同じ手順で関所を通す
7.   全機能が終わり、full の掃引がまだ → /hora-accept を版全体で
```

**各機能の関所18はブラウザ無しのゲート実行で、版はその最後の掃引で一度だけ実際に動かして検証します。** これがこの設計の一文で、掃引がその土台になる実行です。

### 切り替え

**`/hora-fast` へ：いつでも。** `/hora` がゲート途中で残した機能は、未コミットの作業ごと worktree へ移ります。**`/hora` へ戻る：先に「drain」と言います。** 開いている機能をそれぞれのゲートの終わりまで進めてマージし、worktree を閉じます。そのあと `/hora` は通常の開始状態から走ります。

### 何を諦めるか

回帰は、起こした関所ではなくマージの時点で見つかります。画面は各機能のゲートではなく、掃引で一度だけ動かします。複数の機能が同じ時間帯に対話の関所へ到達します。進行中の機能ごとに、2つめの作業コピーと2つめのデータベースが要ります。

---

## `/hora-auto`

**自動経路。** 仕様書のあとの対話を、すべて記録付きの判断に置き換えた `/hora` です。経路の全体は [`auto.ja.md`](./auto.ja.md) にあり、ここはその要約です。何を重視するかを一度だけ宣言すると、実行は `release/<version>` の push と、`main` への draft の pull request まで進みます。

| | |
|---|---|
| **読む** | 走らせるスケジューラが読むものすべて。加えて `.hora/spec/<version>/_stages.md` — ステージ7の通過が記録されている必要がある |
| **書く** | `/hora` と `/hora-build` が書くものすべてを同じ形で。加えて `.hora/tasks/<version>/_auto.md` — 宣言、その変更、すべての停止 — と、代わりに下した判断ごとの `auto-reading` か `auto-default` の question |
| **止まる条件** | 仕様書がステージ7を通っていない。事前点検が通らない。どう決めても解決しない問題が起きた（停止表示がコード付きで示す）。人が「pause」と言った。draft の pull request を出し終えた |
| **単独で叩く** | 常に。再開のときも同じ。`/hora` はこれを起動しませんが、`_auto.md` が `auto: on` の版では、auto のまま続けます |

### 何をするか

```
0. ステージ7を通っているか？    いいえ → 止まる。/hora-spec で仕様書を仕上げる
1. 宣言はあるか？               いいえ → 尋ねる：プロジェクトの性格、スケジューラ、
                                         期日、そのあと直していく順位
2. 事前点検                     実行が必要とするコマンドを、害のない形で一度ずつ
3. スケジューラ本来の経路       問いはすべて振り分ける：読み方、後で緩めやすい側に
                                倒す穴、停止
4. 終点                         release/<version> を push し、main への draft の
                                pull request を出す。マージはしない
```

**諦めるのは人であって、検査ではありません。** 関所、verifier、監査、テストスイートはすべて `/hora` と同じように走り、掃引は full で走ります。

### 止まるときと再開

**止まり方はいつも同じ形で表示されます** — コード、そのあとに、なぜ、どこまで、どう再開するか。示されたものを直して `/hora-auto` をもう一度実行します。`/hora` でも同じように再開します。「pause」で関所の区切りに止まり、「stop auto」で `/hora` に戻ります。

### 何を諦めるか

仕様書の穴は、あなたではなく宣言が決め、question として記録されます。ユースケースは、メインセッションだけでたどります。許容すると決めたかもしれない指摘は、直すか、その機能を止めます。選ぶ人がいないからです。

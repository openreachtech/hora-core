...
主要な6つのコマンドを、毎回同じ5項目で説明します：何をするか / 何を読むか / 何を書くか / いつ止まるか / 単独で叩きたいとき。このほかに直接叩けるものとして、`/hora-hotfix`（緊急経路。後述）、`/bank-id`（このページ末尾）と、`/hora-spec` が走らせる7つのステージ skill（`/hora-spec` の節に名前があります）があります。
...
**緊急の不具合を1つ、`main` へ直接。** 18チェックポイントではなく6つのゲートで通します。受入レビューは諦め、諦めたものを書き残すので、次のバージョンが必ず払い直します。

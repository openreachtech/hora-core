...
**A new project does not install it by hand.** It starts from a GitHub template, and which one depends on what it builds: [`hora-boilerplate`](https://github.com/openreachtech/hora-boilerplate) for a web project, [`hora-boilerplate-mobile`](https://github.com/openreachtech/hora-boilerplate-mobile) for a native Android or iOS one. Each declares this package and the skill libraries its stack needs, places them on `npm install`, and holds the documentation for the kit as a whole — what a project built with it contains, what each command does, and how to put it on a repository that already exists. The method is the same either way; what differs is the stack, the origin catalog, and which skill libraries a row equips. What follows here is for equipping the kit into a repository that came from neither template.
...
How Hora itself works — the method this kit carries — is documented under [`docs/`](./docs/README.md): the quickest route from an empty repository to a spec, what each command does, how the orchestrator runs, putting it on a repository that already exists, the emergency route, the parallel route, and the writing style these documents are held to.
